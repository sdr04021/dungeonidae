...
namespace Gists
{
    ...
    public static class FastPoissonDiskSampling
    {
        ...
        public static List<Vector2> Sampling(Vector2 bottomLeft, Vector2 topRight, float minimumDistance, System.Random rand)
        {
            return Sampling(bottomLeft, topRight, minimumDistance, DefaultIterationPerPoint, rand);
        }

        public static List<Vector2> Sampling(Vector2 bottomLeft, Vector2 topRight, float minimumDistance, int iterationPerPoint, System.Random rand)
        {
            ...
            do
            {
                var index = settings.Rand.Next(0, bags.ActivePoints.Count);
        ...
        {
            var found = false;
            var p = GetRandPosInCircle(set.MinimumDistance, 2f * set.MinimumDistance, set.Rand) + point;
        ...
        private static void GetFirstPoint(Settings set, Bags bags)
        {
            var first = new Vector2(
                ...
                (float)set.Rand.NextDouble() * (set.TopRight.x - set.BottomLeft.x) + set.BottomLeft.x,
                (float)set.Rand.NextDouble() * (set.TopRight.x - set.BottomLeft.x) + set.BottomLeft.x
            );
        ...
        private static Settings GetSettings(Vector2 bl, Vector2 tr, float min, int iteration, System.Random rand)
        {
            var dimension = (tr - bl);
        ...
        private static Vector2 GetRandPosInCircle(float fieldMin, float fieldMax, System.Random rand)
        {
            //var theta = Random.Range(0f, Mathf.PI * 2f);
            //var radius = Mathf.Sqrt(Random.Range(fieldMin * fieldMin, fieldMax * fieldMax));
            var theta = (float)rand.NextDouble() * ((Mathf.PI * 2f) - 0f) + 0f;
            var radius = Mathf.Sqrt((float)rand.NextDouble() * (fieldMax * fieldMax - fieldMin * fieldMin) + fieldMin * fieldMin);
